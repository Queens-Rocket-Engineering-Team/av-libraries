...
bool controlOnRx(Control& c, const aim::Msg& m) {
  ...
  if (c.kind == ControlKind::Local) {
    if (m.cls == Class::Cmd) {
      c.ackSeq     = m.b[0];
      c.pendingAck = true;
      if (m.b[1] <= static_cast<uint8_t>(ControlState::Open)) {
        controlSet(c, m.b[1] == static_cast<uint8_t>(ControlState::Open));
        c.ackResult = AckResult::Accepted;
      } else {
        c.ackResult = AckResult::RejectBadStateValue;
      }
      return true;
    }
    return false;
  ...
  if (m.cls == Class::State) {
    c.state     = (m.b[0] == static_cast<uint8_t>(ControlState::Open));
    c.energized = (m.b[1] == 1U);
    c.hall      = static_cast<ControlState>(m.b[2]);
...
void controlServiceTx(Control& c, uint32_t nowMs, AimNetwork& aim) {
  if (c.kind == ControlKind::Local) {
    ...
    ack.b[0]    = c.ackSeq;
    ack.b[1]    = static_cast<uint8_t>(c.ackResult);
    if (aim.send(ack)) {
      c.pendingAck = false;
    }
    return;
