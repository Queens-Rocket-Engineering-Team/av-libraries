...
struct Control {
  ...
  ControlState hall        = ControlState::Unknown; // Hall sensor state reported on wire
  ...
  AckResult    ackResult   = AckResult::Accepted; // result to echo in that Ack
...
//   Remote — a matching Ack clears the await; a State updates state/energized/hall.
...
// Fill a State frame (b[0]=commanded, b[1]=energized, b[2]=hall) for the owner to broadcast.
