...
class RDESDecompressor():
	...
	def decompress(self, bytes):
		...
		UPDATED TO RETURN ON ANY INDEX ERROR; This may result in the last row
		of the table being lost if it was partially cut off.
		"""
		try:
			## Make temp variables
			self.__lastDecompressed = []
			...
			while (i < len(bytes)):
				...
				if (self.__curCol == self.__numCols):
					self.__curCol = 0
					self.__lastDecompressed.append(curRowVals)
					curRowVals = [0]*self.__numCols
					
		except IndexError:
			pass
		## Return decompressed data
		return self.__lastDecompressed
